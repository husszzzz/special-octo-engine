...
print("⏳ جاري التحقق من ملف اللعبة...")
if not os.path.exists(BASE_IPA):
    try:
        urllib.request.urlretrieve(IPA_URL, BASE_IPA)
        print("✅ تم تحميل اللعبة بنجاح من المستودع!")
    except Exception as e:
        print(f"❌ فشل تحميل اللعبة! يرجى التأكد أن مستودع 8ball-pool (Public) وليس (Private): {e}")
...
def sign_app(chat_id):
    ...
    # التوقيع باستخدام zsign من قلب النظام
    sign_cmd = ["zsign", "-k", data['p12'], "-p", data['password'], "-m", data['prov'], "-o", output_ipa, BASE_IPA]
    result = subprocess.run(sign_cmd, capture_output=True, text=True)
    
    if result.returncode != 0:
        bot.send_message(chat_id, f"❌ فشل التوقيع! تأكد من الباسوورد والشهادة.")
        return

    bot.send_message(chat_id, "✅ تم التوقيع بنجاح! جاري رفع التطبيق وتجهيز الرابط (لأن حجم اللعبة كبير)...")

    try:
        # رفع التطبيق الموقع إلى Gofile والحصول على الرابط
        ...
        bot.send_message(chat_id, f"🎉 **تم توقيع التطبيق!**\n\nحمل اللعبة الموقعة من هنا:\n{download_link}\n\n*الآن خذ هذا الملف وضع رابطه في ملف الـ plist الخاص بك.*", parse_mode="Markdown")
    except Exception as e:
